...
public class VideoModule extends ScalableVideoView implements
  ...
  public void setSrc(final String uriString, final String type, final boolean isNetwork, final boolean isAsset, final ReadableMap requestHeaders, final int expansionMainVersion, final int expansionPatchVersion) {
    ...
    src.putString(VideoHeaderViewManager.PROP_SRC_URI, uriString);
    src.putString(VideoHeaderViewManager.PROP_SRC_TYPE, type);
    src.putMap(VideoHeaderViewManager.PROP_SRC_HEADERS, wRequestHeaders);
    src.putBoolean(VideoHeaderViewManager.PROP_SRC_IS_NETWORK, isNetwork);
    if(mMainVer>0) {
      src.putInt(VideoHeaderViewManager.PROP_SRC_MAINVER, mMainVer);
      if(mPatchVer>0) {
        src.putInt(VideoHeaderViewManager.PROP_SRC_PATCHVER, mPatchVer);
      }
    }
    WritableMap event = Arguments.createMap();
    event.putMap(VideoHeaderViewManager.PROP_SRC, src);
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        if (!mPaused) { // Applying the rate while paused will cause the video to start
          ...
          try {
            ...
            Log.e(VideoHeaderViewManager.REACT_CLASS, "Unable to set rate, unsupported on this device");
          }
        }
      } else {
        Log.e(VideoHeaderViewManager.REACT_CLASS, "Setting playback rate is not yet supported on Android versions below 6.0");
      }
    }
  }
